...
- `perfiles_dieta` — comensales, cobertura (quién come qué y cuándo), reglas y frecuencias de proteína de cada uno, estructura de cada comida, y (si aplica) la **política de comidas preparadas/stock** (p. ej. papillas de un bebé: stock, consumo, frecuencias).
- `pauta_nutricionista_bebe` — **opcional** (solo si un comensal tiene una pauta de especialista, p. ej. un bebé): fuente oficial de esa pauta (estructura del plato, frecuencias, cortes, comidas que cubre).
- `receta_papilla` — **opcional** (solo si el hogar cocina papillas): receta y método de la **tanda**; se consulta **solo al cocinar la papilla** (la política de stock vive en `perfiles_dieta`).
...
- **Dimensiona las cantidades para TODOS los comensales de cada comida**, no solo el núcleo: incluye los **adicionales** según `perfiles_dieta` (p. ej. +1 algunos almuerzos, +2 otro). Cuenta cabezas por comida antes de fijar gramos/porciones. Los adicionales llevan **porción normal**, no la cuota del comensal que tiene una regla propia.
- **Distingue crudo vs. cocido en todo gramaje de proteína** (merma ~25–30 % al cocinar). Las reglas de dieta suelen hablar de gramos **cocidos**; la lista de compras va en **crudo**: dimensiona la compra desde la necesidad cocida (p. ej. 300 g cocidos ≈ 400–430 g crudos) y dilo explícito en las recetas ("300 g cocidos ≈ 430 g crudos").
- **Continuidad entre semanas (potes y stock):** cada plan **define el contenido exacto** de lo que se empaca o congela para la semana siguiente (p. ej. un pote/porción congelada para un día del próximo ciclo, o el stock de papillas de un bebé si aplica) y programa su descongelado. Al planificar, **lee del plan anterior** qué quedó heredado para el ciclo actual, cítalo tal cual y construye el menú sobre ello; si el plan anterior no lo especificó, asúmelo y **márcalo como supuesto**. Nunca dejes un pote/heredado "a definir". **Dentro del ciclo, cada sesión de cocina cocina fresco para los días que abastece; no se "siembran" preparaciones en una sesión para aliviar otra.** Lo único que se arrastra son porciones ya congeladas (un pote de sobra) y, si aplica, el stock de papillas del bebé (ver `perfiles_dieta`).
...
**Instrucciones:** diseña el menú de la semana completa, ramificando por comensal según su dieta y cobertura. **Rota las proteínas parejo por categoría** (vacuno, pollo, pavo/cerdo, pescado, legumbres, huevo…): no cargues la misma proteína en días seguidos, **incluido el fin de semana**. **Rota también los acompañamientos/carbohidratos** (arroz, papa, fideos, puré, quínoa, choclo…): no repitas el mismo carbohidrato en días seguidos ni lo uses por defecto toda la semana — varía entre comidas y comensales. Consulta `bitacora` para no repetir preparaciones ni combinaciones recientes. **Incluye exactamente las comidas que cubre cada comensal según la cobertura de `perfiles_dieta`** — ni más ni menos. El motor **no asume ninguna comida por defecto ni excluye ninguna**: planifica solo las que la cobertura declara (almuerzo, cena, desayuno, colaciones… lo que ese hogar haya definido para cada comensal). Si algún comensal come por **BLW/papilla** (p. ej. un bebé), **cada una de sus comidas se arma con la estructura de plato**: proteína + 1 carbohidrato + ≥3 verduras de colores. No basta con nombrar la proteína (no escribas "BLW huevo" a secas: detalla su carbohidrato y sus verduras). Cuando la comida usa una **papilla del stock** (ya cocinada y congelada), cita su **composición real** — la registrada en `bitacora` o en el plan que cocinó esa tanda —; no inventes acompañamientos que el frasco no trae.
**Referencia:** `perfiles_dieta`, `pauta_nutricionista_bebe`, `organizacion_hogar`, `platos_de_referencia`, `bitacora`.
**Verificaciones clave:** cada comensal cumple las frecuencias y reglas duras de su dieta (cuéntalas y repórtalas); las proteínas quedan balanceadas y **rotadas por categoría** (revisa que no se repita la misma proteína 2 días seguidos, no solo que no se repita el mismo plato); **los acompañamientos/carbohidratos también rotan** (revisa que no se repita el mismo carbohidrato en días seguidos ni que domine toda la semana); **cada comida por BLW/papilla trae carbohidrato + ≥3 verduras** (no solo la proteína); las verduras se nombran concretas (no "sopa verde"); cada comensal usa **una sola proteína por sesión de cocina** aunque su plato varíe entre los días que esa sesión abastece (ver Principios generales); se respeta cualquier verificación específica que defina la dieta de cada comensal.
**Formato de entrega:** tabla `Día | Comida | Plato base | [una columna por comensal]`, con **una fila por cada comida que la cobertura de `perfiles_dieta` define** (almuerzo, cena y cualquier otra que el hogar incluya), desde el inicio del ciclo hasta el día previo a que se vuelva a cocinar; debajo, el resumen de frecuencias por comensal con su verificación.
...
**Instrucciones:** para cada día de cocina, las preparaciones con ingredientes e instrucciones paso a paso, en un orden que aproveche el tiempo y quepa en la sesión. **Cada ingrediente de cada receta lleva su cantidad — en unidades o gramos, idealmente en gramos** (ej. "tortilla: 6 huevos + 1 lata de atún (160 g) + 1 zanahoria (~80 g) + 100 g de espinaca"). **Nada sin cantidad y sin rangos ambiguos** (no "2 o 3 tazas"). Indica también **cuánta proteína** se cocina por comensal, **señalando crudo vs. cocido**. Si algún comensal lleva acompañamientos aparte (p. ej. **verduras BLW al vapor** de un bebé), inclúyelos como paso explícito en la sesión que corresponda, dimensionados para quedar dentro del límite de refri.
**Referencia:** `organizacion_hogar` (días, tiempos, equipos), `receta_papilla`.
**Verificaciones clave:** cada sesión trae un **cronograma en minutos** (ej. 0–10, 10–25…, contando lo que hierve solo en paralelo) y cabe en su **tope de tiempo real** según `organizacion_hogar`; la **sesión más corta** no debe sobrecargarse (si no cabe, **simplifica el menú de esos días**; no aprietes el cronograma en el papel — cada sesión cocina fresco para los días que abastece, no se "siembra" en una sesión para aliviar otra); recetas claras y ejecutivas, paso a paso, **con la cantidad de cada ingrediente (idealmente en gramos)**.
**Formato de entrega:** por día de cocina, lista ordenada de preparaciones con su receta.

### 4. Guardado (qué se guarda y dónde)

**Instrucciones:** define el guardado de cada preparación y hasta cuándo aguanta, separando dos casos: (a) lo que se empaca para llevar, si aplica (ej. un pote de almuerzo para oficina/colegio), indicando si ese pote va a freezer o a refri; (b) el resto de las preparaciones, indicando si van a refri o a freezer. Marca explícitamente lo que va al freezer.
...
1. ☐ **Menú** — **todas las comidas que la cobertura de `perfiles_dieta` define** para cada comensal y día (sin asumir ni excluir ninguna por defecto); comidas por BLW/papilla estructuradas (proteína + 1 carbohidrato + ≥3 verduras de colores); **resumen de frecuencias por comensal con su verificación**.
