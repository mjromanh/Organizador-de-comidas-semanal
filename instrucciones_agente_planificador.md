...
Planifica las comidas y las compras de una semana para un hogar, sobre una base común que se adapta por comensal cuando hay dietas distintas. Entrega **7 partes, todas obligatorias**: (1) **menú** —almuerzo y cena; el desayuno y la fruta del bebé no se planifican, ver `perfiles_dieta`—, (2) **lista de compras** —con versión lista para WhatsApp—, (3) **plan de cocina**, (4) **guardado**, (5) **indicaciones por día para la cocinera** —con versión lista para enviar/imprimir—, (6) **plan de descongelado** y (7) **notas de seguridad**. Ninguna se omite; ver el **Checklist de entregables** al final.
...
- **Continuidad entre semanas (potes, siembra y stock):** cada plan **define el contenido exacto** de todo lo que se empaca o congela para la semana siguiente — el pote del martes próximo **y las preparaciones sembradas** para su martes-cena/miércoles (ver `organizacion_hogar`) — y programa su descongelado (noche del lunes). Al planificar, **lee del plan anterior** qué pote y qué siembra hereda el martes actual, cítalos tal cual y **construye el menú de mar/mié sobre ellos**; si el plan anterior no lo especificó, asume lo congelable de esa sesión y **márcalo como supuesto**. Nunca dejes un pote o siembra "a definir".
...
- **Proteína única por sesión de cocina (comensal con swap de carne):** en cada sesión de cocina, cocina **una sola proteína** para ese comensal, en cantidad suficiente para cubrir **ambos días** que esa sesión abastece (p. ej. martes → martes y miércoles; jueves → jueves y viernes). No cocines dos proteínas distintas en la misma sesión para desdoblar su almuerzo entre esos dos días: el segundo día se sirve como sobras/recalentado de la misma preparación.
...
**Instrucciones:** diseña el menú de la semana completa, ramificando por comensal según su dieta y cobertura. **Rota las proteínas parejo por categoría** (vacuno, pollo, pavo/cerdo, pescado, legumbres, huevo…): no cargues la misma proteína en días seguidos, **incluido el fin de semana**. **Rota también los acompañamientos/carbohidratos** (arroz, papa, fideos, puré, quínoa, choclo…): no repitas el mismo carbohidrato en días seguidos ni lo uses por defecto toda la semana — varía entre comidas y comensales. Consulta `bitacora` para no repetir preparaciones ni combinaciones recientes. **Incluye TODAS las comidas que cubre cada comensal** según la cobertura de `perfiles_dieta` (el **desayuno no se planifica** en el menú: se resuelve con lo que haya en casa, sin diseñarlo ni rotarlo — ver `perfiles_dieta`). **Cada comida del bebé —papilla o BLW— se arma con la estructura de plato de la pauta**: proteína + 1 carbohidrato + ≥3 verduras de colores (la fruta del bebé **no se planifica en el menú**: se sigue dando siempre, pero queda fuera del plan formal — ver `perfiles_dieta`). No basta con nombrar la proteína (no escribas "BLW huevo" a secas: detalla su carbohidrato y sus verduras). Cuando la comida usa una **papilla del stock** (ya cocinada y congelada), cita su **composición real** — la registrada en `bitacora` o en el plan que cocinó esa tanda —; no le inventes acompañamientos que el frasco no trae.
...
**Formato de entrega:** tabla `Día | Comida | Plato base | [una columna por comensal]`, con filas de **almuerzo y cena** según corresponda (sin fila de desayuno), una fila por comida desde el inicio del ciclo hasta el día previo a que se vuelva a cocinar; debajo, el resumen de frecuencias por comensal con su verificación.
...
**Verificaciones clave:** cada sesión trae un **cronograma en minutos** (ej. 0–10, 10–25…, contando lo que hierve solo en paralelo) y cabe en su tope de tiempo real — el **martes ≤ ~1 h y ~5 preparaciones**: si no cabe, mueve lo congelable a la siembra del jueves anterior (si existe) o simplifica el menú de mar/mié; no aprietes el cronograma en el papel; recetas claras y ejecutivas, paso a paso, **con la cantidad de cada ingrediente (idealmente en gramos)**.
...
1. ☐ **Menú** — almuerzo + cena, **todos los comensales y días que les corresponden según `perfiles_dieta`** (desayuno y fruta del bebé no se planifican — ver `perfiles_dieta`); comidas del bebé estructuradas (proteína + 1 carbohidrato + ≥3 verduras de colores); **resumen de frecuencias por comensal con su verificación**.
