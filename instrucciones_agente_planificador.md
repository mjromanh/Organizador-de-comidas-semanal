...
Planifica las comidas y las compras de una semana para un hogar, sobre una base común que se adapta por comensal cuando hay dietas distintas. Entrega **7 partes, todas obligatorias**: (1) **menú** —almuerzo y cena; el desayuno y la fruta del bebé no se planifican **ni se muestran en el plan**, ver `perfiles_dieta`—, (2) **lista de compras** —con versión lista para WhatsApp—, (3) **plan de cocina**, (4) **guardado**, (5) **indicaciones por día para la cocinera** —con versión lista para enviar/imprimir—, (6) **plan de descongelado** y (7) **notas de seguridad**. Ninguna se omite; ver el **Checklist de entregables** al final.
...
- **Dimensiona las cantidades para TODOS los comensales de cada comida**, no solo el núcleo: incluye los **adicionales** según `perfiles_dieta` (p. ej. +1 algunos almuerzos, +2 otro). Cuenta cabezas por comida antes de fijar gramos/porciones. Los adicionales llevan **porción normal** (~150–200 g de proteína cocida), no la cuota del comensal que tiene regla propia.
- **Distingue crudo vs. cocido en todo gramaje de proteína** (merma ~25–30 % al cocinar). Las reglas de dieta hablan de gramos **cocidos**; la lista de compras va en **crudo**: dimensiona la compra desde la necesidad cocida (p. ej. 300 g cocidos ≈ 400–430 g crudos) y dilo explícito en las recetas ("300 g cocidos", "1,1 kg crudos ≈ 800 g cocidos").
- **Continuidad entre semanas (pote de oficina y stock):** cada plan **define el contenido exacto** del **pote de oficina del martes siguiente** — una porción **congelada de la base del jueves** (la base del jueves alcanza hasta el viernes y el resto se congela) — y programa su descongelado (noche del lunes). Al planificar, **lee del plan anterior** de qué base sale el pote del martes actual y cítalo tal cual; si el plan anterior no lo especificó, asúmelo y **márcalo como supuesto**. Nunca dejes un pote "a definir". **El martes cocina fresco su base y su sopa; no hay "siembra" de preparaciones de una semana a la otra.** (El stock de papillas del bebé sí se arrastra entre semanas — ver `perfiles_dieta`.)
...
- **Proteína única por sesión de cocina (comensal con swap de carne):** en cada sesión de cocina, cocina **una sola proteína** para ese comensal, en cantidad suficiente para cubrir **todos los días que esa sesión abastece** (la del martes alcanza hasta el jueves; la del jueves, jueves y viernes). No cocines dos proteínas distintas en la misma sesión para desdoblar su almuerzo: los días siguientes se sirven como sobras/recalentado de la misma preparación.
...
**Instrucciones:** diseña el menú de la semana completa, ramificando por comensal según su dieta y cobertura. **Rota las proteínas parejo por categoría** (vacuno, pollo, pavo/cerdo, pescado, legumbres, huevo…): no cargues la misma proteína en días seguidos, **incluido el fin de semana**. **Rota también los acompañamientos/carbohidratos** (arroz, papa, fideos, puré, quínoa, choclo…): no repitas el mismo carbohidrato en días seguidos ni lo uses por defecto toda la semana — varía entre comidas y comensales. Consulta `bitacora` para no repetir preparaciones ni combinaciones recientes. **Incluye TODAS las comidas que cubre cada comensal** según la cobertura de `perfiles_dieta` (el **desayuno NO va en el plan**: no incluyas ninguna tabla ni sección de desayunos, **ni siquiera como referencia** — el hogar lo resuelve aparte, ver `perfiles_dieta`; el menú arranca directo en almuerzo). **Cada comida del bebé —papilla o BLW— se arma con la estructura de plato de la pauta**: proteína + 1 carbohidrato + ≥3 verduras de colores, **sin fruta** (la fruta del bebé **NO va en el plan**: no la escribas en sus comidas ni como sugerencia/nota; el hogar la da aparte, ver `perfiles_dieta`). No basta con nombrar la proteína (no escribas "BLW huevo" a secas: detalla su carbohidrato y sus verduras). Cuando la comida usa una **papilla del stock** (ya cocinada y congelada), cita su **composición real** — la registrada en `bitacora` o en el plan que cocinó esa tanda —; no le inventes acompañamientos que el frasco no trae.
**Referencia:** `perfiles_dieta`, `pauta_nutricionista_bebe`, `organizacion_hogar`, `platos_de_referencia`, `bitacora`.
**Verificaciones clave:** cada comensal cumple las frecuencias y reglas duras de su dieta (cuéntalas y repórtalas); las proteínas quedan balanceadas y **rotadas por categoría** (revisa que no se repita la misma proteína 2 días seguidos, no solo que no se repita el mismo plato); **los acompañamientos/carbohidratos también rotan** (revisa que no se repita el mismo carbohidrato en días seguidos ni que domine toda la semana); **cada BLW/papilla del bebé trae carbohidrato + ≥3 verduras** (no solo la proteína; la fruta no se planifica en el menú, ver `perfiles_dieta`); las verduras se nombran concretas (no "sopa verde"); el comensal con swap de carne usa **una sola proteína por sesión de cocina** (ver Principios generales); se respeta cualquier verificación específica que defina la dieta de cada comensal.
**Formato de entrega:** tabla `Día | Comida | Plato base | [una columna por comensal]`, con filas de **almuerzo y cena** según corresponda (**sin fila ni sección de desayuno; el plan arranca directo en almuerzo/cena**), una fila por comida desde el inicio del ciclo hasta el día previo a que se vuelva a cocinar; debajo, el resumen de frecuencias por comensal con su verificación.
...
**Verificaciones clave:** marca/formato según archivo; cantidades cubren el menú; **cobertura cruzada menú↔lista:** cada ingrediente que el menú usa aparece en la lista o lleva nota explícita de "hay stock" — nada implícito. Las notas de "hay stock" **solo valen** para lo declarado en `ajustes_semana` y para aliños básicos de despensa (aceite, especias, sal); **todo lo demás se lista con su costo aunque se haya comprado la semana anterior** (la persona lo descarta en el carrito — no asumas que quedó); costo estimado por ítem, **subtotal por sección** y **total = suma aritmética real de los ítems** (verifícalo, no lo estimes a ojo). El total debe estar **bajo el tope**; el rango de `organizacion_hogar` es **referencia, no límite** (si se supera la referencia pero se respeta el tope, está OK; si se supera el tope, avísalo).
...
**Instrucciones:** para cada día de cocina, las preparaciones con ingredientes e instrucciones paso a paso, en un orden que aproveche el tiempo y quepa en la sesión. **Cada ingrediente de cada receta lleva su cantidad — en unidades o gramos, idealmente en gramos** (ej. "tortilla: 6 huevos + 1 lata de atún (160 g) + 1 zanahoria (~80 g) + 100 g de espinaca"). **Nada sin cantidad y sin rangos ambiguos** (no "2 o 3 tazas"). Indica también **cuánta proteína** se cocina por comensal, **señalando crudo vs. cocido**. Incluye como paso explícito la cocción de los acompañamientos del bebé (p. ej. **verduras BLW al vapor**) en la sesión que corresponda, dimensionadas para quedar dentro del límite de refri.
**Referencia:** `organizacion_hogar` (días, tiempos, equipos), `receta_papilla`.
**Verificaciones clave:** cada sesión trae un **cronograma en minutos** (ej. 0–10, 10–25…, contando lo que hierve solo en paralelo) y cabe en su tope de tiempo real — el **martes (~1 h) = una base con variaciones por comensal (alcanza hasta el jueves) + una sopa de verduras**; el **jueves (1–2 h) = una base con variaciones (hasta el viernes) + la papilla + una legumbre** (ver `organizacion_hogar`): si algo no cabe, simplifica; no aprietes el cronograma en el papel; recetas claras y ejecutivas, paso a paso, **con la cantidad de cada ingrediente (idealmente en gramos)**.
...
1. ☐ **Menú** — almuerzo + cena, **todos los comensales y días que les corresponden según `perfiles_dieta`** (desayuno y fruta del bebé **no aparecen en el plan** — ni tabla de desayunos ni fruta/sugerencias; ver `perfiles_dieta`); comidas del bebé estructuradas (proteína + 1 carbohidrato + ≥3 verduras de colores, **sin fruta**); **resumen de frecuencias por comensal con su verificación**.
